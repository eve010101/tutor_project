...
function normalizeGradeRanges(values?: string[] | null, legacyValue?: string | null) {
  if (values?.length) {
...
function normalizeAvailableTimeSlots(values?: string[] | null, legacyValue?: string | null) {
  if (values?.length) {
...
function normalizeServiceAreas(values?: string[] | null, legacyValue?: string | null) {
  if (values?.length) {
    return filterByOptions(values, BEIJING_DISTRICT_OPTIONS);
  }

  return filterByOptions(parseLegacyList(legacyValue), BEIJING_DISTRICT_OPTIONS);
}
...
export function TutorProfileForm({ profile, tutorProfile }: TutorProfileFormProps) {
  ...
  const [form, setForm] = useState<TutorProfileFormState>({
    ...
    subjects: filterByOptions(tutorProfile?.subjects ?? [], TUTOR_SUBJECT_OPTIONS),
    ...
    hourlyRate: tutorProfile?.hourly_rate ? String(tutorProfile.hourly_rate) : "",
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="grid gap-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2 sm:col-span-2">
                  ...
                  />
                  <p className="text-xs text-slate-500">仅平台留存，不对家长展示全名。</p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(event) => setField("gaokaoOrigin", event.target.value)}
                  />
                </div>
              </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(event) => setField("weeklyCapacity", event.target.value)}
                />
              </div>
            </div>
                    ...
                    <div className="grid gap-3 sm:grid-cols-3">
                      {group.options.map((option) => (
                        <SelectionButton
                          ...
                          onClick={() => toggleMultiValue("availableTimeSlots", option)}
                        />
                      ))}
                    </div>
                  </div>
          ...
          <section className="space-y-5 rounded-[28px] border border-slate-200 bg-white p-5 sm:p-6">
            <div className="space-y-1">
              ...
              <p className="text-sm text-slate-500">
                学信网截图仅用于平台审核，不会对家长展示。重新上传截图并保存后，审核状态会回到待审核。
              </p>
            </div>
            ...
            <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-5">
              <div className="grid gap-4 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
                <div
                  className={cn(
                    "min-h-[220px] rounded-3xl border border-slate-200 bg-white bg-contain bg-center bg-no-repeat",
                  ...
                  style={
                    verificationPreviewUrl
                      ? { backgroundImage: `url("${verificationPreviewUrl}")` }
                      : undefined
                  }
                >
                  {verificationPreviewUrl ? null : <UploadCloud className="h-8 w-8" />}
                </div>

                <div className="space-y-3">
                  <Label>学信网截图</Label>
                  <p className="text-sm leading-6 text-slate-600">
                    请上传包含学校和在读信息的截图。当前仅平台审核可见，家长端不会展示。
                  </p>
                  <p className="text-xs leading-5 text-slate-500">
                    建议打码敏感证件号，只保留学校、身份和在读关键信息。
                  </p>
                  <input
                    accept="image/*"
                    className="hidden"
                  ...
                  <Button
                    ...
                    variant="outline"
                  >
                    {verificationPreviewUrl ? "重新上传截图" : "上传学信网截图"}
                  </Button>
                </div>
              </div>
            </div>
